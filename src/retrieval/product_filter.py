...
class ProductFilter:
    ...
    PRODUCT_KEYWORDS = {
        "продукти": ["продукти", "каталог", "стоки", "асаортимент", "какви стоки", "какво предлагате", "какви продукти", "какви продукти предлагате"],
    ...
    def extract_keywords(query: str) -> List[str]:
        ...
        # Check for non-existent products (helicopters, airplanes, etc.)
        non_existent_products = [
            r"\bимате ли\s+(хеликоптери?|самолети?|ракети?|подводници?|танкове?|магически пръчки?)\b",
            r"\bпродавате ли\s+(хеликоптери?|самолети?|ракети?|подводници?|танкове?|магически пръчки?)\b",
            r"\bпредлагате ли\s+(хеликоптери?|самолети?|ракети?|подводници?|танкове?|магически пръчки?)\b"
        ]
        
        # If query contains non-existent products, don't match as products
        for pat in non_existent_products:
            if re.search(pat, q):
                return []  # Return empty list - no product keywords
